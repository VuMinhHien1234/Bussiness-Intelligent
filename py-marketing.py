...
# Nguồn 2: Facebook Ads CSV.
FB_ADS_CSV_PATH = os.getenv("FB_ADS_CSV_PATH", "fb_ads_spend.csv")
if not os.path.isabs(FB_ADS_CSV_PATH):
    FB_ADS_CSV_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), FB_ADS_CSV_PATH)
FB_ADS_RAW_TABLE = os.getenv("FB_ADS_RAW_TABLE", "fb_ads_csv")
FACT_AD_SPEND_TABLE = os.getenv("FACT_AD_SPEND_TABLE", "fact_ad_spend")

# Nguồn 3: thời tiết (Open-Meteo API -> weather_daily.csv).
WEATHER_CSV_PATH = os.getenv("WEATHER_CSV_PATH", "weather_daily.csv")
if not os.path.isabs(WEATHER_CSV_PATH):
    WEATHER_CSV_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), WEATHER_CSV_PATH)
WEATHER_RAW_TABLE = os.getenv("WEATHER_RAW_TABLE", "weather_csv")
FACT_WEATHER_TABLE = os.getenv("FACT_WEATHER_TABLE", "fact_weather_daily")

# Dimension mới: vùng, chi nhánh, sản phẩm.
DIM_REGION_TABLE = os.getenv("MARKETING_DIM_REGION_TABLE", "dim_region")
DIM_BRANCH_TABLE = os.getenv("MARKETING_DIM_BRANCH_TABLE", "dim_branch")
DIM_PRODUCT_TABLE = os.getenv("MARKETING_DIM_PRODUCT_TABLE", "dim_product")

# Tên chiến dịch bên hệ thống ads (slug) -> tên chuẩn trong dim_campaign,
# để 2 nguồn dùng chung dimension (conformed dimensions).
CAMPAIGN_SLUG_MAP = {
...
def load_to_staging(conn, excel_path):
    ...
    df = pd.read_excel(excel_path, dtype=str)  # keep everything as text for staging
    df = df.where(pd.notnull(df), None)         # NaN → None for psycopg2

    with conn.cursor() as cur:
        cur.execute(f"TRUNCATE {table_name(STG_SCHEMA, RAW_TABLE)}")
        rows = [tuple(row) for row in df.itertuples(index=False, name=None)]
        if rows:
            col_list = ", ".join(f'"{c}"' for c in df.columns)
            execute_values(
                cur,
                f'INSERT INTO {table_name(STG_SCHEMA, RAW_TABLE)} ({col_list}) VALUES %s',
                rows,
            )
    conn.commit()
    print(f"Staging loaded: {len(df)} rows")
...
def clean_data(df):
    ...
    # 5) Log chất lượng dữ liệu, rồi loại các dòng không phân tích được.
    reasons = {
        "order_id lỗi/thiếu": df["order_id"].isna(),
        "ngày lỗi/thiếu (sai định dạng)": df["date"].isna(),
        "thiếu channel": df["channel"].isna(),
        "thiếu stage": df["stage"].isna(),
        "thiếu revenue": df["revenue"].isna(),
    }
    bad = pd.Series(False, index=df.index)
    for label, mask in reasons.items():
        n = int((mask & ~bad).sum())
        if n:
            print(f"  - Loại {n} dòng: {label}")
        bad |= mask
    df = df[~bad]
    ...
    if before_dup - len(df):
        print(f"  - Gộp {before_dup - len(df)} dòng trùng OrderID (giữ bản ghi mới nhất)")
...
def load_dimensions(conn, df):
    with conn.cursor() as cur:
        ...
        # Dim vùng / chi nhánh / sản phẩm (phục vụ phân tích theo địa lý và ngành hàng).
        execute_values_if_any(
            cur,
            f"""
        ...
        execute_values_if_any(
            cur,
            f"""
            INSERT INTO {table_name(DW_SCHEMA, DIM_PRODUCT_TABLE)} (product_name, category_name)
            VALUES %s
            ON CONFLICT (product_name) DO UPDATE SET
                category_name = EXCLUDED.category_name
            """,
            to_records(product_pairs),
        )
...
def load_ads_to_staging(conn, csv_path):
    """Đọc CSV Facebook Ads và nạp nguyên trạng vào staging (truncate + reload)."""
    print(f"Loading {csv_path} → {STG_SCHEMA}.{FB_ADS_RAW_TABLE}")
    df = pd.read_csv(csv_path, dtype=str)
    df = df.where(pd.notnull(df), None)
    with conn.cursor() as cur:
        cur.execute(f"TRUNCATE {table_name(STG_SCHEMA, FB_ADS_RAW_TABLE)}")
        rows = [tuple(row) for row in df.itertuples(index=False, name=None)]
        if rows:
            col_list = ", ".join(f'"{c}"' for c in df.columns)
            execute_values(
                cur,
                f'INSERT INTO {table_name(STG_SCHEMA, FB_ADS_RAW_TABLE)} ({col_list}) VALUES %s',
                rows,
            )
    conn.commit()
...
def clean_ads_data(df):
    """Làm sạch ads: parse ngày dd/mm/yyyy, map tên chiến dịch về chuẩn, ép kiểu số."""
    df = df.copy()
    before = len(df)
    df["date"] = parse_dates(df["date"])
    df["campaign"] = (
        df["campaign"].astype("string").str.strip().str.lower().map(CAMPAIGN_SLUG_MAP)
    )
    for col in ["spend", "impressions", "clicks", "conversions"]:
        df[col] = pd.to_numeric(df[col], errors="coerce")
    df = df.dropna(subset=["date", "campaign", "spend"])
    # Gộp về đúng grain: 1 dòng = 1 ngày x 1 chiến dịch.
    ...
    print(f"Ads rows: {before} → {len(df)} (đã loại dòng lỗi, gộp trùng)")
    return df


def load_fact_ad_spend(conn, ads_df):
    """Nạp fact_ad_spend; bổ sung các giá trị dim còn thiếu vì dim dùng chung 2 nguồn."""
    dates = pd.to_datetime(ads_df["date"])
    dim_date = pd.DataFrame({
        "full_date": ads_df["date"],
        "year": dates.dt.year,
    ...
    campaign_rows = [
        (campaign, classify_campaign(campaign))
        for campaign in ads_df["campaign"].drop_duplicates()
    ]

    with conn.cursor() as cur:
        # Ngày chỉ chạy quảng cáo (không có đơn hàng) vẫn phải tồn tại trong dim_date.
        execute_values_if_any(
            cur,
            f"""
            INSERT INTO {table_name(DW_SCHEMA, DIM_DATE_TABLE)}
        ...
        cur.execute(
            f"""
            INSERT INTO {table_name(DW_SCHEMA, DIM_CHANNEL_TABLE)} (channel_name, channel_type)
        ...
        execute_values_if_any(
            cur,
            f"""
            INSERT INTO {table_name(DW_SCHEMA, FACT_AD_SPEND_TABLE)} (
...
def load_weather_to_staging(conn, csv_path):
    """Đọc weather_daily.csv (do fetch_weather.py tạo) vào staging."""
    print(f"Loading {csv_path} → {STG_SCHEMA}.{WEATHER_RAW_TABLE}")
    df = pd.read_csv(csv_path, dtype=str)
    df = df.where(pd.notnull(df), None)
    with conn.cursor() as cur:
        cur.execute(f"TRUNCATE {table_name(STG_SCHEMA, WEATHER_RAW_TABLE)}")
        rows = [tuple(row) for row in df.itertuples(index=False, name=None)]
        if rows:
            col_list = ", ".join(f'"{c}"' for c in df.columns)
            execute_values(
                cur,
                f'INSERT INTO {table_name(STG_SCHEMA, WEATHER_RAW_TABLE)} ({col_list}) VALUES %s',
                rows,
            )
    conn.commit()
...
def clean_weather_data(df):
    """Làm sạch thời tiết: parse ngày, ép kiểu số, đúng grain 1 ngày x 1 vùng."""
    df = df.copy()
    before = len(df)
    df["date"] = parse_dates(df["date"])
    ...
    print(f"Weather rows: {before} → {len(df)}")
    return df


def load_fact_weather(conn, wdf):
    """Nạp fact_weather_daily; bổ sung dim_date/dim_region còn thiếu (dim dùng chung)."""
    dates = pd.to_datetime(wdf["date"])
    dim_date = pd.DataFrame({
        "full_date": wdf["date"],
        "year": dates.dt.year,
        "month": dates.dt.month,
    ...
    with conn.cursor() as cur:
        ...
        execute_values_if_any(
            ...
            ON CONFLICT (region_name) DO NOTHING
            """,
            [(clean_value(x),) for x in wdf["region"].dropna().drop_duplicates()],
        )
        fact_df = wdf[["date", "region", "temp_mean", "temp_max", "rain_mm"]]
        execute_values_if_any(
            cur,
            f"""
            INSERT INTO {table_name(DW_SCHEMA, FACT_WEATHER_TABLE)} (
...
def main():
    ...
    with psycopg2.connect(**DB_CONFIG) as conn:
        ...
        # Step 1: Load Excel → staging
        load_to_staging(conn, EXCEL_PATH)

        # Step 2: Read from staging
        ...
        # Step 3: Nguồn 2 — Facebook Ads CSV → staging → fact_ad_spend
        load_ads_to_staging(conn, FB_ADS_CSV_PATH)
        ads_raw = pd.read_sql(
            f"SELECT * FROM {table_name(STG_SCHEMA, FB_ADS_RAW_TABLE)}", conn
        )
        print(f"Ads raw rows: {len(ads_raw)}")
        ads_df = clean_ads_data(ads_raw)
        load_fact_ad_spend(conn, ads_df)

        # Step 4: Nguồn 3 — Thời tiết (Open-Meteo) → staging → fact_weather_daily
        if os.path.exists(WEATHER_CSV_PATH):
            load_weather_to_staging(conn, WEATHER_CSV_PATH)
            weather_raw = pd.read_sql(
                f"SELECT * FROM {table_name(STG_SCHEMA, WEATHER_RAW_TABLE)}", conn
            )
            weather_df = clean_weather_data(weather_raw)
            load_fact_weather(conn, weather_df)
        else:
            print(f"Bỏ qua nguồn thời tiết: chưa có {os.path.basename(WEATHER_CSV_PATH)} "
                  "(chạy 'python3 fetch_weather.py' trước).")

        print("Marketing ETL finished")
        print(f"Loaded dimensions and facts into schema: {DW_SCHEMA}")
        print(f"Fact tables: {FACT_MARKETING_TABLE}, {FACT_AD_SPEND_TABLE}")
